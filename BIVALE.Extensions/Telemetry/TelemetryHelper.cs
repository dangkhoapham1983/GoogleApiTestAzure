...
namespace BIVALE.Extensions.Telemetry
{
	public class TelemetryHelper : ITelemetryHelper
	{
		...
		private static readonly Lazy<TelemetryMetadata> metadata;
		...
		private static Lazy<ITelemetryHelper> current;
		...
		public TelemetryHelper()
		{
			...
			this.client = new Lazy<TelemetryClient>(
				() =>
				{
					var key = CloudConfigurationManager.GetSetting(TelemetryMetadata.AppSettingKey);
		...
		static TelemetryHelper()
		{
			...
			current = new Lazy<ITelemetryHelper>(() => new TelemetryHelper());
		}
